    ...
    try:
        async with managed_page() as (_context, _page):
            ...
            try:
                            ...
                            try:
                                ...
                                async with httpx.AsyncClient(timeout=30.0) as client:
                                    for key in keys_to_try:
                                        dl_url = f"{base_url}/storage/v1/object/public/{bucket}/{key}"
                ...
                for step_num in range(1, MAX_FORM_STEPS + 1):
                    ...
                    if not form_fields:
                        if step_num == 1:
                            ...
                            bound_log.info("pipeline.no_further_steps_detected", completed_step=step_num - 1)
                            final_status = RunStatus.SUBMITTED
                            artifact_label = "confirmation"
                            break

                    bound_log.info(
                        "pipeline.fields_extracted",
                ...
                # Save artifacts WHILE PAGE IS STILL ACTIVE AND OPEN
        ...
        final_status = RunStatus.FAILED
        error_reason = f"Browser launch failure: {launch_exc}"
        bound_log.exception("pipeline.browser_launch_failed", error=str(launch_exc))
